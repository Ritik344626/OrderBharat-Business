...
export class AppadsaddComponent implements OnInit {
  ...
  priceCalculation(start_date, end_date, seletedKm) {
    ...
    let perLocation = Math.round(this.days * this.priceFix(seletedKm));

    // location selection based on final price
    let price = perLocation * this.selectedLocation.length;
    this.discount = 0;
    this.maxprice = price;
  ...
  priceFix(kmRange: any): number {
    //first 1 km
    if (kmRange == 1) {
      return 16.67;
    } else {
      //first 1 km above calculation
      return 1 * 16.67 + (kmRange - 1) * 15;
    }
  }
